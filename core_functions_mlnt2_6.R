...
load_data = function(normalise = TRUE, remove_doublets = FALSE, remove_stripped = FALSE, load_corrected = FALSE){
  ...
  counts = readRDS("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT01/clustering/raw_counts.rds")
  genes = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT01/clustering/features.tsv", stringsAsFactors = F)
  meta = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT02/output/test/meta7_cheesed.tab", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "$")
  ...
  if(normalise){
    sfs = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT01/clustering/sizefactors_v3.tab", stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs
  ...
  if(load_corrected){
    corrected = readRDS("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT01/batchcorrection/corrected_pcas.rds")
...
getHVGs <- function(sce, min.mean = 1e-3, chrY.genes.file = "/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT01/cell_qualitycontrol/ygenes.tab", sparse_matrix = FALSE){
